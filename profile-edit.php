			...
			<div class="row" >
				...
				<div class="col-sm-offset-2 col-sm-6">
					<form method="post" class="form-group">
						...
						<input name="user_nicename" type="text" class="form-control" value="<?php echo $user->user_nicename; ?>" />
						<label for="email" class="control-label">Email Address:</label>
						<input name="user_email" type="email" class="form-control" value="<?php echo $user->user_email; ?>" />
						<label for="password" class="control-label">Password:</label>
						<input name="user_pass" type="password" class="form-control" value="<?php echo $user->user_pass; ?>" />
						<label for="user_info" class="control-label">Info:</label>
						<textarea name="user_info" type="text" class="form-control" style="resize:none"> <?php echo $user->user_info; ?></textarea>
						<input type="submit" class="btn btn-primary pull-right" value="Submit" />
					</form>
					<form action="upload-image.php" method="POST" enctype="multipart/form-data">
						<?php if (isset($_SESSION['image_upload'])){
							if ($_SESSION['image_upload'] == "Success"){
								?> <div class="alert alert-success" role="alert"><?php  echo $_SESSION['image_upload']; ?> </div> <?php
							} else {
								?> <div class="alert alert-danger" role="alert"><?php  echo $_SESSION['image_upload'][0]; ?> </div> <?php
							}
							unset($_SESSION['image_upload']);
						}?>
						<input name="profile_id" type="hidden" value="<?php echo $logged_user_id; ?>" />
